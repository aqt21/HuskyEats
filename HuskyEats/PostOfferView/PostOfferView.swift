    ...
    @IBAction func offerClick(_ sender: UIButton) {
        ...
        if let finalOffer = offerText, let offerAmount = Double(finalOffer) {
            ...
            if let priceClarified = price, let priceInDouble = Double(priceClarified){
                currRef?.setValue(["restaurant": selectedRestaurant, "item": selectedItem, "price": self.itemPrice.text, "offerPercent": userOffer.text, "offerUser": userID, "offerPrice": offerAmount / 100 * priceInDouble])
            }
        } else {
